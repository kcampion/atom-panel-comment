    ...
    if (this.rightPanel.isVisible()) {
      ...
      return (this.rightPanel.hide());
		...
		editor.onDidChangeCursorPosition(function(event){
			...
			var commentaryLines = item.editorElement.getElementsByClassName("syntax--comment syntax--line syntax--double-slash syntax--js")
			function notPonctuation(element) {
				if (element.classList.value.includes('syntax-ponctuation') || element.innerText == "//") {
					return false;
				}
				return element;
			}
			...
			const commentaryBlocks = item.editorElement.getElementsByClassName("syntax--comment syntax--block syntax--documentation syntax--js");
			...
			const textBlocks = commentaryBlocksArray.map(textBlocks => ({
				text: textBlocks.innerText,
			 	})
		 	)

			console.log("cursor change", textBlocks)
		 	var range = editor.getCurrentParagraphBufferRange() // give a range
		 	if (range !== undefined && range.start.row >= 5) {
				...
				for (var tex of text) {
					if (paragraph.includes(tex.text)) {
						...
						// console.log(tex.text + "Yes")
						// add_text(tex.text);
					}
				}
				for (var textblock of textBlocks) {
					if (paragraph.includes(textblock.text)) {
						that.pannelCommentView.add_text(textblock.text)
						// console.log(tex.text + "Yes")
						// add_text(tex.text);
					}
				}
